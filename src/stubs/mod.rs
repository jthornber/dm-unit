...
pub fn standard_globals(fix: &mut Fixture) -> Result<()> {
    ...
    let _ = fix.at_func("dm_block_data", Arc::new(bm_block_data));
    let _ = fix.at_func("dm_block_location", Arc::new(bm_block_location));
    let _ = fix.at_func("dm_block_manager_create", Arc::new(bm_create));
    let _ = fix.at_func("dm_block_manager_destroy", Arc::new(bm_destroy));
    let _ = fix.at_func("dm_bm_block_size", Arc::new(bm_block_size));
    let _ = fix.at_func("dm_bm_checksum", Arc::new(bm_checksum));
    let _ = fix.at_func("dm_bm_flush", Arc::new(bm_flush));
    let _ = fix.at_func("dm_bm_is_read_only", Arc::new(bm_is_read_only));
    let _ = fix.at_func("dm_bm_nr_blocks", Arc::new(bm_nr_blocks));
    let _ = fix.at_func("dm_bm_prefetch", Arc::new(bm_prefetch));
    let _ = fix.at_func("dm_bm_forget", Arc::new(bm_forget));
    let _ = fix.at_func("dm_bm_unlock_move", Arc::new(bm_unlock_move));
    let _ = fix.at_func("dm_bm_read_lock", Arc::new(bm_read_lock));
    let _ = fix.at_func("dm_bm_read_try_lock", Arc::new(bm_read_lock));
    let _ = fix.at_func("dm_bm_set_read_only", Arc::new(bm_set_read_only));
    let _ = fix.at_func("dm_bm_set_read_write", Arc::new(bm_set_read_write));
    let _ = fix.at_func("dm_bm_unlock", Arc::new(bm_unlock));
    let _ = fix.at_func("dm_bm_write_lock", Arc::new(bm_write_lock));
    let _ = fix.at_func("dm_bm_write_lock_zero", Arc::new(bm_write_lock_zero));
    let _ = fix.at_func("kfree", Arc::new(kfree));
    let _ = fix.at_func("__kmalloc", Arc::new(kmalloc));
    let _ = fix.at_func("kmalloc_order", Arc::new(kmalloc));
    let _ = fix.at_func("memcpy", Arc::new(memcpy));
    let _ = fix.at_func("memmove", Arc::new(memcpy));
    let _ = fix.at_func("memcmp", Arc::new(memcmp));
    let _ = fix.at_func("memset", Arc::new(memset));
    let _ = fix.at_func("printk", Arc::new(printk));
    let _ = fix.at_func("strncpy", Arc::new(strncpy));
    let _ = fix.at_func("i_size_read__", Arc::new(i_size_read));
